...
def create_buffer(request, distance=0.01):
    # 获取站点的查询集，包含id, name, latitude, longitude 字段
    stations = Stations.objects.all()
    data = []
    # 遍历每个站点，并处理地理空间数据
    for station in stations:
        ...
        # 创建缓冲区
        buffer_point = point.buffer(distance)
        buffer_geojson = mapping(buffer_point)  # 转换为GeoJSON格式
        data.append({
            'id': station.id,
            'name': station.name,
            'latitude': latitude,
            'longitude': longitude,
            'location': mapping(point),  # 转换点对象为GeoJSON
            'buffer_point': buffer_geojson  # 缓冲区对象转换为GeoJSON
        })
    # 返回 JSON 响应
    return JsonResponse(data, safe=False)
