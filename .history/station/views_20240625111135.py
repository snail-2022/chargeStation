...
from .point import danZhongXingFa
from .point2 import juLeiFangFa
import pandas as pd
...
def get_point_data(request):
    ...
    for station in stations:
        station_data = {
            ...
            'total_charging_points': station.total_charging_points,
            'active_charging_points': station.active_charging_points,
            'electricity_price': station.electricity_price,
            'power': station.power,
            'build_time': station.build_time,
            'maintenance_time': station.maintenance_time,
        }
...
def get_data(request):
    stations = ChargingStations.objects.all().values('latitude', 'longitude')
    data = pd.DataFrame(list(stations))
    data['weight'] = 1
    center_longitude, center_latitude=danZhongXingFa(data)

    center_station = ChargingStations.objects.create(
        name = "newPoint",
...
def get_data2(request):
    stations = ChargingStations.objects.all().values('latitude', 'longitude')
    data = pd.DataFrame(list(stations))
    cluster_centers=juLeiFangFa(data)
    for center in cluster_centers:
        ChargingStations.objects.create(
            name="newpoint2",
            longitude=center['longitude'],
            latitude=center['latitude']
        )
    return JsonResponse(cluster_centers ,safe=False)

def HeatmapPage(request):
    return render(request, "缓冲区_2.html")


def dapingPage(request):
