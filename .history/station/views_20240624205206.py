...
def create_charging_station(request):
    if request.method == 'POST':
        ...
        # 创建新的充电站对象
        charging_station = Stations.objects.create(
...
def search_charging_stations(request):
    if request.method == 'GET':
        ...
        charging_stations = Stations.objects.filter(name__icontains=query)
        # 构造包含查询结果的列表
...
def update_charging_station(request, station_id):
    if request.method == 'PUT':
        try:
            # 根据充电站 ID 查询充电站数据
            charging_station = Stations.objects.get(pk=station_id)
            # 更新充电站信息
...
def get_data(request):
    stations = Stations.objects.all().values('latitude', 'longitude')
    data = pd.DataFrame(list(stations))
    data['weight'] = 1
    center_longitude, center_latitude=danZhongXingFa(data)

    center_station = Stations.objects.create(
    name = "newPoint",
